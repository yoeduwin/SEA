...
//   3. fase3_CrearExpediente()   → resolución exacta RFC+sucursal, sin fallback a raíz
...
const CONFIG = {
  ...
  FOLDER_STRUCTURE: {
    ...
    CROQUIS:         '3. CROQUIS',
    FOTOS:           '4. FOTOS',
    INFORMES_MEMORIA:'5. INFORMES Y MEMORIAS',
    INF_PRELIMINAR:  '6. INFORME PRELIMINAR'
  },
...
// =========================================================================
// FASE 1: REGISTRO CLIENTE (22 Columnas; contrato existente)
// =========================================================================
function fase1_RegistrarCliente(data) {
  ...
  try {
    ...
    // Preflight de sólo lectura: si el contrato existente no está disponible,
    ...
    // PAIC es el mismo alta/actualización de cliente-sucursal que SEAPD; lo único
    // propio del portal es que el acuse va al asesor, no al cliente final.
    if (esPaic) {
      if (!String(data.correo_acuse || '').trim()) {
        return { success: false, error: 'El correo del asesor/intermediario es obligatorio en PAIC.' };
      }
      if (['si','no'].indexOf(String(data.aplica_nom020 || '').toLowerCase()) === -1) {
        return { success: false, error: 'Indique si este registro requiere NOM-020.' };
      }
    }
    ...
    if (pIter.hasNext()) {
      ...
      while (allParents.hasNext()) {
        ...
        if (isParentFolderForRfc_(candidate.getName(), rfcClean)) {
          parentFolder = candidate;
          addLog(`Carpeta Padre reutilizada por RFC: ${candidate.getName()}`);
          break;
        }
      }
    }
    ...
    if (bIter.hasNext()) {
      ...
      addLog(`Carpeta Sucursal encontrada: ${branchClean}`);
    } else {
      carpetaCliente = parentFolder.createFolder(branchClean);
      addLog(`Carpeta Sucursal creada: ${branchClean}`);
    }

    // 3. Perfil del cliente. No movemos archivos históricos: sólo las nuevas
    // cargas se organizan en /Sucursal/01_Cliente.
    let carpetaPerfil;
    const perfilIter = carpetaCliente.getFoldersByName('01_Cliente');
    carpetaPerfil = perfilIter.hasNext() ? perfilIter.next() : carpetaCliente.createFolder('01_Cliente');
    const processedFiles = guardarArchivos(data, carpetaPerfil, addLog);
    const sheetUrl = generarPerfilSheet(data, carpetaPerfil, companyClean, branchClean, timestamp, addLog);
    ...
    for (let i = allData.length - 1; i >= 1; i--) {
      if (String(allData[i][CL.RFC]).toUpperCase().trim() === rfcClean &&
          sanitizeFileName(String(allData[i][CL.SUCURSAL] || 'Matriz')).toLowerCase() === branchClean.toLowerCase()) {
        rowIndex = i + 1;
        break;
      }
    }
    // PAIC no captura la indicación de PIPC. Al actualizar una sucursal existente
    // conservamos el valor que ya tiene la fila para no degradar a "NO" un dato
    // que el asesor no ve ni declara (p. ej. capturado antes desde SEAPD).
    if (esPaic && rowIndex > -1) {
      rowData[CL.REQUIERE_PIPC] = allData[rowIndex - 1][CL.REQUIERE_PIPC] || 'NO';
    }
    if (rowIndex > -1) {
    ...
    guardarLogEnDrive(carpetaPerfil, logEntries, data);
    return {
      success: true,
      message: 'Registro/Actualización exitosa',
      files: processedFiles.length,
      linkDrive: carpetaCliente.getUrl()
    };
  } catch (error) {
    ...
    if (rfcFila === rfcBuscado.toUpperCase().trim()) {
      ...
      if (!setSucursalesUnicas.has(nombreSucursal)) {
        ...
        sucursalesEncontradas.push({
          ...
          capacidad_instalada:  data[i][CL.CAP_INSTALADA],
          capacidad_operacion:  data[i][CL.CAP_OPERACION],
          dias_turnos_horarios: data[i][CL.DIAS_TURNOS],
          aplica_nom020:        data[i][CL.APLICA_NOM020],
          requiere_pipc:        data[i][CL.REQUIERE_PIPC],
          responsable:          data[i][CL.RESPONSABLE],
          telefono_responsable: data[i][CL.TEL_RESPONSABLE],
          nombre_dirigido:      data[i][CL.NOMBRE_DIRIGIDO],
          puesto_dirigido:      data[i][CL.PUESTO_DIRIGIDO],
          link_drive_cliente:   data[i][CL.LINK_DRIVE],
          asesor_consultor:     data[i][CL.ASESOR_CONSULTOR]
        });
      }
    }
  }
...
// =========================================================================
// Resolución segura de la carpeta del cliente:
//   1) Coincidencia exacta RFC+sucursal en CLIENTES_MAESTRO.
//   2) Link de OT/payload sólo si carpeta y padre coinciden con sucursal/RFC.
//   3) Ruta exacta en Drive. Si no existe, se detiene sin crear en raíz.
...
function isParentFolderForRfc_(folderName, rfc) {
  ...
  if (!expectedRfc || name.indexOf(expectedRfc) !== 0) return false;
  if (name.length === expectedRfc.length) return true;
...
function findExistingExpediente_(sheetInf, ot) {
  if (!sheetInf) return { found: false };
  const rows = sheetInf.getDataRange().getValues();
  const normalizedOt = normalizeOtForSeainf_(ot);
  for (let i = rows.length - 1; i >= 1; i--) {
    if (normalizeOtForSeainf_(rows[i][CI.OT]) !== normalizedOt) continue;
    const link = String(rows[i][CI.LINK_DRIVE] || '');
    return {
      found: true,
      link: link,
      folder: getFolderByDriveLinkSafe_(link),
      sheetRow: i + 1
    };
  }
  return { found: false };
...
function ensureExpedienteSubfolders_(expedienteFolder) {
  const names = CONFIG.FOLDER_STRUCTURE;
  const folders = {};
  const existing = expedienteFolder.getFolders();
  while (existing.hasNext()) {
    const folder = existing.next();
    Object.keys(names).forEach(function(key) {
      if (!folders[key] && folder.getName() === names[key]) folders[key] = folder;
    });
  }
  Object.keys(names).forEach(function(key) {
    if (!folders[key]) folders[key] = expedienteFolder.createFolder(names[key]);
  });
  return folders;
}
...
function validateDriveFiles_(files) {
  ...
  (files || []).forEach(function(file) {
    if (!file || !file.content) {
      rejected.push({ name: file && file.name ? file.name : 'archivo', error: 'Contenido vacío.' });
      return;
    }
    const check = validarArchivo_(file.content, file.type || '', file.name || 'archivo');
    if (!check.valid) {
      rejected.push({ name: file.name || 'archivo', error: check.error });
      return;
    }
    valid.push(file);
  });
...
function fase3_CrearExpediente(payload) {
  ...
  sheetInf.appendRow([
    ...
    info.telefono || '',
    info.direccion || '',
    info.fechaServicio || otRow[CO.FECHA_VISITA] || '',
    otRow[CO.FECHA_ENTREGA] || '',
    info.esCapacitacion || 'NO',
    'NO INICIADO',
    expedienteFolder.getUrl(),
    otRow[CO.PERSONAL] || '',
    otRow[CO.SUCURSAL] || ''
  ]);
...
// enlace de sucursal que será validado nuevamente antes de crear el expediente
// =========================================================================
  ...
  fileFields.forEach(field => {
    ...
    if (fileData && fileName && typeof fileData === 'string' && fileData.startsWith('data:')) {
      try {
        ...
        // Renombrar el archivo con base en el texto de la celda correspondiente
        const ext = fileName.includes('.') ? fileName.split('.').pop() : '';
        let baseName;
        if (field.key === 'ine_testigo1') {
          baseName = data.testigo1 ? `B) INE Testigo 1 - ${data.testigo1}` : 'B) INE Testigo 1';
        } else if (field.key === 'ine_testigo2') {
          baseName = data.testigo2 ? `C) INE Testigo 2 - ${data.testigo2}` : 'C) INE Testigo 2';
        } else {
          baseName = field.label;
        }
        const finalName = ext ? `${sanitizeFileName(baseName)}.${ext}` : sanitizeFileName(baseName);
        const blob = Utilities.newBlob(Utilities.base64Decode(parts[1]), mimeType, finalName);
        const file = carpetaCliente.createFile(blob);
        files.push({ name: finalName, label: field.label, url: file.getUrl(), size: data[field.key + '_size'] || 0 });
        if (addLog) addLog(`  - Archivo guardado: ${finalName}`);
...
function generarPerfilSheet(data, carpetaCliente, cleanedCompany, cleanedBranch, timestamp, addLog) {
  ...
  if (data.aplica_nom020) {
    setHeader(`A${currentRow+5}:L${currentRow+5}`, 'NOM-020-STPS');
    sheet.getRange(`B${currentRow+6}:L${currentRow+6}`).merge().setValue(data.aplica_nom020 === 'si' ? 'SÍ APLICA' : 'NO APLICA').setHorizontalAlignment('center').setFontWeight('bold');
    if (data.aplica_nom020 === 'si') {
      fillFieldCell(`B${currentRow+7}:C${currentRow+7}`, 'Jefe de Mantenimiento / Responsable:', `D${currentRow+7}`, data.jefe_mantenimiento || '', { wrap: true });
      setHeader(`A${currentRow+9}:L${currentRow+9}`, 'TESTIGOS NOM-020-STPS');
      fillFieldCell(`B${currentRow+10}:C${currentRow+10}`, 'Testigo 1 (Nombre y Puesto):', `D${currentRow+10}`, data.testigo1 || '', { wrap: true });
      fillFieldCell(`B${currentRow+12}:C${currentRow+12}`, 'Testigo 2 (Nombre y Puesto):', `D${currentRow+12}`, data.testigo2 || '', { wrap: true });
    }
  }
...
function enviarNotificacionEquipoPaic(data, files, carpetaCliente, sheetUrl) {
  ...
  const subject = `Registro PAIC · ${data.razon_social || 'Cliente'} — ${data.sucursal || 'Sucursal'}`;
  const plain = [
    'REGISTRO DE CLIENTE / SUCURSAL VÍA PAIC','',
    ...
    'Este mensaje confirma el registro o la actualización de un cliente/sucursal vía asesor. No significa que exista una OT; las órdenes de trabajo se generan por separado en SEAOT.'
  ].join('\n');
  const html = envolturaEmail_(
    `Registro PAIC · ${escHtml_(data.razon_social || '')} · ${escHtml_(data.sucursal || '')}`,
    encabezadoEmail_({ kicker:'Registro vía PAIC', titulo:escHtml_(data.razon_social || 'Cliente final'), subtitulo:`Sucursal: ${valorOGuion_(data.sucursal)}`, metas:[{etiqueta:'Recibido',valor:timestamp},{etiqueta:'RFC',valor:valorOGuion_(data.rfc)}] }) +
...
function enviarConfirmacionPaic(data, carpetaCliente, files) {
  ...
  const plain = [
    ...
    `Indicación NOM-020: ${valorSiNoPaic_(data.aplica_nom020)}`,'',
    'Este acuse confirma la recepción de los datos y la documentación enviados a través de PAIC. No confirma ni genera una orden de trabajo; las OT se gestionan por separado con Ejecutiva Ambiental.','',
    `Atención a Clientes: ${CONFIG.SUPPORT_EMAIL} · ${CONFIG.SUPPORT_PHONE}`
  ].join('\n');
  const html = envolturaEmail_(
    `Recibimos su registro PAIC · ${escHtml_(data.razon_social || '')}`,
    ...
    `<tr><td style="padding:26px 30px;"><p style="margin:0 0 18px;font-size:15px;line-height:1.7;color:${EMAIL_COLORS_.texto};">Este mensaje confirma la recepción del registro realizado por usted como asesor/intermediario.</p><table width="100%" border="0" cellpadding="0" cellspacing="0">${filaEmail_('Cliente final',valorOGuion_(data.razon_social))}${filaEmail_('Sucursal',valorOGuion_(data.sucursal))}${filaEmail_('RFC',valorOGuion_(data.rfc))}${filaEmail_('Documentos recibidos',String(docsCount))}${filaEmail_('Indicación NOM-020',valorSiNoPaic_(data.aplica_nom020))}</table>${separadorEmail_()}<div style="background-color:${EMAIL_COLORS_.panel};border:1px solid ${EMAIL_COLORS_.borde};border-radius:8px;padding:16px;font-size:13px;line-height:1.6;color:${EMAIL_COLORS_.textoSuave};">El registro PAIC no genera una orden de trabajo. Las OT se gestionan posteriormente y por separado con Ejecutiva Ambiental.</div></td></tr>` +
...
function enviarEmailSimpleFallbackPaic(data, carpetaCliente, sheetUrl) {
  const subject=`Registro PAIC · ${data.razon_social || 'Cliente'} — ${data.sucursal || 'Sucursal'}`;
  const body=`Registro de cliente/sucursal vía PAIC.\nCliente: ${data.razon_social || '-'}\nSucursal: ${data.sucursal || '-'}\nRFC: ${data.rfc || '-'}\nAsesor: ${data.nombre_solicitante || '-'}\nCorreo asesor: ${data.correo_acuse || '-'}\nCarpeta: ${carpetaCliente.getUrl()}\nPerfil: ${sheetUrl}\n\nNo implica creación de OT.`;
  GmailApp.sendEmail(CONFIG.EMAIL_TO.join(','),subject,body);
  const asesor=String(data.correo_acuse || '').trim();
  if (asesor) GmailApp.sendEmail(asesor,`Registro PAIC recibido · ${data.razon_social || 'Cliente'}`,`Recibimos el registro del cliente ${data.razon_social || '-'} / ${data.sucursal || '-'}. Este acuse no genera una OT; la OT se gestiona por separado.`);
}
 ...
 */
function normalizedMimeType_(mimeType, fileName) {
...
function validarArchivo_(content, mimeType, fileName) {
  ...
  if (!content) return { valid: false, error: 'Archivo "' + fileName + '" sin contenido.' };
  if (estimatedBytes > MAX_BYTES) {
    return { valid: false, error: 'Archivo "' + fileName + '" supera el límite de 50 MB.' };
  }

  const ext = String(fileName || '').toLowerCase().split('.').pop();
  const normalizedMime = normalizedMimeType_(mimeType, fileName);
  if (ALLOWED_EXTENSIONS.indexOf(ext) === -1 || ALLOWED_MIMES.indexOf(normalizedMime) === -1) {
    return {
      valid: false,
      error: 'Tipo de archivo no permitido: "' + fileName + '" (' + (mimeType || 'sin MIME') + ').'
    };
  }
